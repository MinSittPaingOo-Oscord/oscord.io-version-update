...
if ($conn) {
    ...
    $stmt = mysqli_prepare($conn, "
        SELECT b.*, c.id AS courseID, c.name AS course_name 
        FROM batch b 
...
// Fetch Modules & Items (same as moduleNotFree.php)
if ($courseID) {
...
<head>
    ...
    <title><?= htmlspecialchars($batch['course_name'] ?? 'Group Class') ?> - Oscord</title>
    ...
    <style>
        ...
        button[onclick="nextLesson()"] {
            ...
            font-weight: 600;
            box-shadow: 0 4px 12px rgba(26, 22, 18, 0.3);
        }
    ...
    <div id="sidebar" class="sidebar w-80 bg-white border-r flex flex-col overflow-hidden md:static">
        <div class="p-5 border-b flex items-center justify-between md:hidden">
            <div>
                ...
                <div class="font-display text-lg font-light"><?= htmlspecialchars($batch['course_name']) ?></div>
            </div>
            <button onclick="toggleSidebar()" class="text-3xl leading-none hover:bg-gray-100 w-10 h-10 flex items-center justify-center rounded-xl">✕</button>
        </div>
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
        ...
        <div class="h-14 bg-white border-b flex items-center px-6 justify-between flex-shrink-0">
            <div class="flex items-center gap-4">
                ...
                <a href="batches.php" class="flex items-center gap-2 text-sm hover:text-var(--gold)">
                    <i class="fa-solid fa-arrow-left"></i>
                    <span class="hidden md:inline">Back to Batches</span>
                </a>
            </div>
            <h1 class="font-display text-lg font-light"><?= htmlspecialchars($batch['course_name']) ?> - Batch <?= $batch['batchNumber'] ?></h1>
            <div></div>
        </div>
        ...
        <div class="bg-white border-t p-5 flex-shrink-0">
            ...
            <div class="flex gap-3 mt-4">
                ...
                <button onclick="nextLesson()" 
                        class="flex-1 py-4 bg-[#1A1612] hover:bg-black text-white rounded-2xl flex items-center justify-center gap-2 transition-all font-medium shadow-md">
