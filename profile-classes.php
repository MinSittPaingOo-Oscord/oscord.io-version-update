<?php
// profile-classes.php - MOBILE RESPONSIVE
...
<div class="max-w-5xl mx-auto">
    <h2 class="text-3xl sm:text-5xl font-light tracking-[-1px] sm:tracking-[-2px] mb-6 sm:mb-12">Your Classes</h2>
    
    <?php if (empty($groupEnrollments)): ?>
        <div class="bg-white border-2 border-black p-8 sm:p-12 text-center">
            <p class="text-lg sm:text-xl text-zinc-500">No active classes at the moment.</p>
            <p class="mt-3 sm:mt-4 text-zinc-400 text-sm sm:text-base">Your group classes and batch schedules will appear here.</p>
            <a href="batches.php" 
               class="inline-block mt-8 sm:mt-10 px-8 sm:px-10 py-4 sm:py-5 bg-black text-white text-base sm:text-lg font-medium hover:bg-purple-600 transition-colors">
    ...
    <?php else: ?>
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-8">
            <?php foreach ($groupEnrollments as $row): ?>
                ...
                <div class="group bg-white border-2 border-black hover:border-purple-600 transition-all duration-300 p-5 sm:p-8 flex flex-col h-full">
                    
                    <div class="mb-4 sm:mb-6">
                        <p class="text-xs uppercase tracking-[0.5px] text-zinc-400 mb-1">Batch #<?= htmlspecialchars($row['batchNumber']) ?></p>
                        <h3 class="text-xl sm:text-2xl font-light tracking-tight leading-snug line-clamp-2">
                            <?= htmlspecialchars($row['course_name']) ?>
                        </h3>
                    </div>

                    <div class="flex flex-wrap gap-2 sm:gap-3 mb-5 sm:mb-8">
                        <div class="inline-flex items-center px-3 sm:px-5 py-1.5 text-xs font-medium border <?= $approveColor ?> rounded-none">
                            <?= $approveLabel ?>
                        </div>
                        <div class="inline-flex items-center px-3 sm:px-5 py-1.5 text-xs font-medium border border-black/70 bg-white rounded-none">
                    ...
                    <div class="mt-auto grid grid-cols-2 gap-4 sm:gap-8 text-sm mb-6 sm:mb-10">
                        <div>
                            <p class="text-xs uppercase tracking-[0.5px] text-zinc-400 mb-1">Starts</p>
                            <p class="text-zinc-600 font-light text-sm sm:text-base">
                    ...
                    <?php if ($isApproved): ?>
                        <a href="specificGroupClass.php?batchID=<?= (int)$row['batchID'] ?>" 
                           class="mt-auto block w-full text-center py-4 sm:py-5 text-base sm:text-lg font-medium border-2 border-black hover:bg-purple-600 hover:text-white hover:border-purple-600 transition-all">
                            GET INTO CLASS →
                        </a>
                    <?php else: ?>
                        <button onclick="showWaitModal()" 
                                class="mt-auto block w-full text-center py-4 sm:py-5 text-base sm:text-lg font-medium border-2 border-amber-400 bg-amber-50 text-amber-700 cursor-default">
                            Wait for Approval
                        </button>
                    <?php endif; ?>
                </div>
...
<div id="waitModal" class="hidden fixed inset-0 bg-black/70 flex items-center justify-center z-50 px-4">
    <div class="bg-white rounded-3xl p-6 sm:p-8 max-w-md w-full text-center">
        <i class="fa-solid fa-clock text-4xl sm:text-5xl text-amber-500 mb-4 sm:mb-6"></i>
        <h2 class="font-display text-2xl sm:text-3xl mb-3">Wait for Approval</h2>
        <p class="text-gray-600 mb-6 sm:mb-8 text-sm sm:text-base">Your enrollment is under review.<br>Admin will approve it shortly.</p>
        <button onclick="hideWaitModal()" 
                class="px-8 sm:px-10 py-3 sm:py-4 bg-black text-white rounded-2xl font-medium hover:bg-zinc-800 transition">
            OK
        </button>
    </div>
</div>
